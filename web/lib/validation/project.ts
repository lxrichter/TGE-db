...
export function validateProjectForm(
  ...
  if (isBlank(input.project_phase)) {
    errors.push({ field: "project_phase", message: "Project Phase is required." });
  }
  ...
  const numericFields: Array<[keyof ProjectValidationInput, string, string, boolean]> = [
  ...
  const plannedCodYear = codYear(input.cod);
