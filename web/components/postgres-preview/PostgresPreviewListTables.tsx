...
export type PreviewActiveFilter = {
  label: string;
  value: string;
};
...
export function previewFilterOptionLabel(
  options: PreviewFilterOption[],
  value: string | undefined
) {
  if (!value) {
  ...
  return (
    ...
    formatPreviewFilterLabel(value)
  );
}
...
export function activePreviewQuickView({
  currentQuery,
  views,
}: {
  currentQuery: Record<string, string | undefined>;
  ...
  return (
    views.find((view) =>
      isQuickViewActive({ currentQuery, viewQuery: view.query })
    ) || null
  );
}

function activeFilterSummaryLabel(filters: PreviewActiveFilter[]) {
  if (filters.length === 0) {
    return "No active filters";
  }

  return `${formatCount(filters.length)} active filter${
    filters.length === 1 ? "" : "s"
  }`;
}

export function PostgresPreviewListContext({
  entityLabel,
  shownCount,
  total,
  page,
  ...
  page: number;
  pageSize: number;
  activeFilters: PreviewActiveFilter[];
  activeViewLabel: string;
  exportHref: string;
}) {
  const start = total === 0 || shownCount === 0 ? 0 : (page - 1) * pageSize + 1;
