...
import PromoteProjectButton from "@/components/projects/PromoteProjectButton";
import {
  canEdit,
  canApprove,
  type UserRole,
...
function classifyCompanyRow(row: ProjectCompanyRow) {
  const role = normalizeRole(row.role);
  const hasOwnership = row.ownership_share !== null && row.ownership_share !== undefined;
...
function getGroupName(row: ProjectCompanyRow) {
  const role = normalizeRole(row.role);
...
function groupPrimaryCompanies(rows: ProjectCompanyRow[]) {
...
function groupSecondaryCompanies(rows: ProjectCompanyRow[]) {
  const grouped = new Map<string, ProjectCompanyRow[]>();
...
function PrimaryCompaniesCompact({ rows }: { rows: ProjectCompanyRow[] }) {
...
function SecondaryCompaniesCompact({
  ...
  groups: { groupName: string; items: ProjectCompanyRow[] }[];
}) {
...
function OtherCompaniesCompact({ rows }: { rows: ProjectCompanyRow[] }) {
  if (!rows.length) return null;
...
export default async function ProjectDetailPage({
  ...
  const project = (await db.get(
    ...
    `,
    id
  )) as ProjectDetailRow | undefined;

  const projectCompanies = (await db.all(
    `
    SELECT
      l.company_project_link_id,
      l.company_id,
      c.company_name,
      l.role,
      l.role_detail,
            ...
            <div className="print-header">
              <div className="print-header-main">
                ...
                <div className="print-title-block">
                  <div className="print-type">Project Overview</div>
                  <h1 className="print-title">Project not found</h1>
                </div>
              </div>
            </div>
  ...
  const primaryCompanies = projectCompanies.filter(
    (row) => classifyCompanyRow(row) === "Primary"
  );

  const secondaryCompanies = projectCompanies.filter(
    (row) => classifyCompanyRow(row) === "Secondary"
  );

  const otherCompanies = projectCompanies.filter(
    (row) => classifyCompanyRow(row) === "Other"
  );

  const secondaryGroups = groupSecondaryCompanies(secondaryCompanies);

  const latitude = toNumber(project.location_x);
  const longitude = toNumber(project.location_y);
    ...
    <>
      <main className="screen-only space-y-6">
        ...
        <section className="border border-gray-200 bg-white">
            ...
            <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
              <div className="max-w-5xl">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  <ResearchBadge value={project.research_status} />
              ...
              <div className="flex w-full max-w-[880px] flex-col gap-3 xl:w-auto xl:min-w-[640px] xl:items-end">
                <div className="flex flex-wrap gap-2 xl:justify-end">
                  ...
                  {userCanApprove ? (
                    <ApproveProjectButton projectId={project.project_id} />
                  ) : null}
                </div>
                ...
                <div className="flex flex-wrap gap-3 xl:justify-end">
                  ...
                  <PromoteProjectButton
                    ...
                    disabled={Boolean(project.is_promoted_to_plant)}
                    promotedPlantId={project.promoted_plant_id ?? null}
                  />
                </div>
              </div>
            </div>
          ...
          <div className="border-t border-gray-200 bg-[#fafafa] px-6 py-4">
            <div className="grid grid-cols-2 gap-x-6 gap-y-5 xl:grid-cols-[120px_160px_180px_minmax(240px,1.4fr)_140px]">
              ...
              <SummaryItem
                label="Technology"
                value={displayValue(project.plant_technology)}
              />
              <SummaryItem
                label="Operator"
                value={displayValue(project.owner_operator)}
              />
              <SummaryItem
                label="Phase"
                value={displayValue(project.project_phase)}
              />
            </div>
          </div>
        </section>
            ...
            <div className="px-5 py-4 text-[13px] leading-6 text-gray-600">
              {project.edited_description ? (
                ...
                <p>
                  ...
                  short project description, internal summary, or validated market
              ...
              <div className="p-0">
                {activeTab === "ownership-location" && (
                  <TabSection title="Location & Legacy Fields">
                    ...
                    <Row label="Project group" value={project.project_group} />
                    <Row label="Other name" value={project.other_name} />
                    <Row label="Legacy owner / operator" value={project.owner_operator} />
                    <Row label="Legacy developer" value={project.developer} />
                    <Row
                      label="Location (city/province)"
                      value={project.location_text}
                    />
                    <Row label="Country" value={project.country} />
                    <Row label="Region" value={project.region} />
                    <Row label="WB region" value={project.wb_region} />
                  </TabSection>
                )}
                ...
                {activeTab === "capacity-timeline" && (
                  <TabSection title="Capacity & Timeline">
                    ...
                    />
                    <Row
                      label="Planned capacity running"
                      value={project.capacity_running_mw}
                    />
                    <Row
                      label="Gross production (GWh)"
                      value={project.gross_production_gwh}
                    />
                    <Row
                      label="Start of dev. (year)"
                      value={project.start_dev_year}
                    />
                    <Row label="Planned COD" value={project.cod} />
                  </TabSection>
                )}

                {activeTab === "resource-status" && (
                  <TabSection title="Resource & Project Status">
                    <Row label="Resource type" value={project.resource_type} />
                    <Row
                      label="Resource temp. (°C)"
                      value={project.resource_temp_c}
                    />
                    <Row
                      label="Project phase"
                      value={<PhaseBadge value={project.project_phase} />}
                    />
                    <Row
                      label="Phase (historical)"
                      value={<PhaseBadge value={project.phase_historical} />}
                    />
                    <Row label="Field name" value={project.field_name} />
                  </TabSection>
                )}

                {activeTab === "wellfield" && (
                  <TabSection title="Wellfield Data">
                    <Row label="# Wells total" value={project.wells_total} />
                    <Row
                      label="# Wells prod. active"
                      value={project.wells_prod_active}
                    />
                    <Row
                      label="# Wells reinj active"
                      value={project.wells_reinj_active}
                    />
                    <Row
                      label="# Wells inactive / standby"
                      value={project.wells_inactive_standby}
                    />
                    <Row
                      label="# Wells other / explor."
                      value={project.wells_other_exploration}
                    />
                    <Row
                      label="Well depth prod. (m)"
                      value={project.well_depth_prod_m}
                    />
                    <Row
                      label="Temp. prod. well (°C)"
                      value={project.temp_prod_well_c}
                    />
                    <Row label="Flow rate (l/s)" value={project.flow_rate_ls} />
                  </TabSection>
                )}

                {activeTab === "plant-commercial" && (
                  <TabSection title="Plant / Technology / Commercial">
                    <Row label="Number of unit" value={project.number_of_unit} />
                    <Row
                      label="Plant technology"
                      value={project.plant_technology}
                    />
                    <Row
                      label="Turbine supplier"
                      value={project.turbine_supplier}
                    />
                    <Row label="EPC / suppliers" value={project.epc_suppliers} />
                    <Row label="Legacy investor" value={project.investor} />
                    <Row label="PPA, USD / kWh" value={project.ppa_usd_kwh} />
                    <Row
                      label="Total investment cost"
                      value={project.total_investment_cost}
                    />
                  </TabSection>
                )}
              </div>
          ...
          <div className="space-y-6">
            <section className="border border-gray-200 bg-white">
              ...
              <div className="px-5 py-3">
                <Row
                  label="Review status"
                  value={<ReviewStatusBadge value={project.review_status} />}
                />
                <Row
                  label="Research status"
                  value={<ResearchBadge value={project.research_status} />}
                />
                <Row label="Location X" value={project.location_x} />
                <Row label="Location Y" value={project.location_y} />
                <Row
                  label="Created by"
                ...
                <Row
                  ...
                  value={displayUserName(
                    project.last_updated_by_name,
                    project.last_updated_by_user_id
                  )}
                />
                <Row
                  label="Date created"
                  value={formatDisplayDate(project.date_created)}
                />
                <Row
                  label="Date edited"
                  value={formatDisplayDate(project.date_edited)}
                />
                <Row
                  label="Approved by"
                  value={displayUserName(
                    project.approved_by_name,
                    project.approved_by_user_id
                  )}
                />
                <Row
                  label="Approved on"
                  value={formatDisplayDate(project.approved_at)}
                />
              </div>
            </section>
          </div>
        ...
        <section className="border border-gray-200 bg-white">
          ...
          <div className="px-5 py-3">
            <Row label="Notes" value={project.notes} labelWidth="170px" />
            <Row
              label="Website / information"
              value={project.website_information}
              labelWidth="170px"
            />
          </div>
        </section>
        ...
        <div className="print-doc">
          <div className="print-header">
            <div className="print-header-main">
              ...
              <div className="print-title-block">
                <div className="print-type">Project Overview</div>
                <h1 className="print-title">{project.project_name || "NA"}</h1>
                <p className="print-subtitle">
                  {project.country || "NA"} | {project.region || "NA"} | {project.location_text || "NA"}
                </p>
              </div>
            </div>
            ...
            <div className="print-summary-grid">
              <div className="print-summary-item">
                <div className="print-summary-label">Project ID</div>
                <div className="print-summary-value">{project.project_id || "NA"}</div>
              </div>
              ...
              <div className="print-summary-item">
                ...
                <div className="print-summary-value">{project.plant_technology || "NA"}</div>
              </div>

              <div className="print-summary-item">
                <div className="print-summary-label">Operator</div>
                <div className="print-summary-value">{project.owner_operator || "NA"}</div>
              </div>

              <div className="print-summary-item">
                <div className="print-summary-label">Phase</div>
                <div className="print-summary-value">{project.project_phase || "NA"}</div>
              </div>
          ...
          <section className="print-section">
            ...
            <p className="print-paragraph">
              {project.edited_description ||
                "No editorial summary added yet. This space can later hold a short project description, internal summary, or validated market narrative for quick understanding and future edit workflows."}
            </p>
          </section>
          ...
          <section className="print-section">
            ...
            <div className="print-grid">
              <div className="print-row"><span>Project ID</span><strong>{project.project_id || "NA"}</strong></div>
              <div className="print-row"><span>Project name</span><strong>{project.project_name || "NA"}</strong></div>
              <div className="print-row"><span>Project group</span><strong>{project.project_group || "NA"}</strong></div>
              <div className="print-row"><span>Other name</span><strong>{project.other_name || "NA"}</strong></div>
              <div className="print-row"><span>Legacy owner / operator</span><strong>{project.owner_operator || "NA"}</strong></div>
              <div className="print-row"><span>Legacy developer</span><strong>{project.developer || "NA"}</strong></div>
              <div className="print-row"><span>Location</span><strong>{project.location_text || "NA"}</strong></div>
              <div className="print-row"><span>Country</span><strong>{project.country || "NA"}</strong></div>
              <div className="print-row"><span>Region</span><strong>{project.region || "NA"}</strong></div>
              <div className="print-row"><span>WB region</span><strong>{project.wb_region || "NA"}</strong></div>
            </div>
          </section>
          ...
          <section className="print-section">
            ...
            <div className="print-grid">
              ...
              <div className="print-row"><span>Start of dev. (year)</span><strong>{project.start_dev_year || "NA"}</strong></div>
              <div className="print-row"><span>Planned COD</span><strong>{project.cod || "NA"}</strong></div>
            </div>
          </section>
          ...
          <section className="print-section">
            ...
            <div className="print-grid">
              <div className="print-row"><span>Resource type</span><strong>{project.resource_type || "NA"}</strong></div>
              <div className="print-row"><span>Resource temp. (°C)</span><strong>{project.resource_temp_c ?? "NA"}</strong></div>
              <div className="print-row"><span>Project phase</span><strong>{project.project_phase || "NA"}</strong></div>
              <div className="print-row"><span>Phase historical</span><strong>{project.phase_historical || "NA"}</strong></div>
              <div className="print-row"><span>Field name</span><strong>{project.field_name || "NA"}</strong></div>
            </div>
          </section>
          ...
          <section className="print-section">
            ...
            <div className="print-grid">
              <div className="print-row"><span>Number of unit</span><strong>{project.number_of_unit || "NA"}</strong></div>
              <div className="print-row"><span>Plant technology</span><strong>{project.plant_technology || "NA"}</strong></div>
              <div className="print-row"><span>Turbine supplier</span><strong>{project.turbine_supplier || "NA"}</strong></div>
              <div className="print-row"><span>EPC / suppliers</span><strong>{project.epc_suppliers || "NA"}</strong></div>
              <div className="print-row"><span>Legacy investor</span><strong>{project.investor || "NA"}</strong></div>
              <div className="print-row"><span>PPA, USD / kWh</span><strong>{project.ppa_usd_kwh || "NA"}</strong></div>
              <div className="print-row"><span>Total investment cost</span><strong>{project.total_investment_cost || "NA"}</strong></div>
            </div>
          </section>
          ...
          <section className="print-section">
            ...
            <div className="print-grid">
              <div className="print-row"><span>Notes</span><strong>{project.notes || "NA"}</strong></div>
              <div className="print-row"><span>Website / information</span><strong>{project.website_information || "NA"}</strong></div>
            </div>
          </section>
