  ...
  try {
    ...
    const duplicate = await db.get(
      ...
      WHERE company_id_from = ?
        ...
        AND company_relationship_id <> ?
      `,
      [company_id_from, company_id_to, relationship_type, id]
    );
    ...
    console.error("PUT /api/company-relationships/[id] error:", error);
    return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    ...
    const existing = await db.get(
      `SELECT company_relationship_id FROM company_relationships WHERE company_relationship_id = ?`,
      [id]
    );
    ...
    await db.run(
      `DELETE FROM company_relationships WHERE company_relationship_id = ?`,
      [id]
    );

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error("DELETE /api/company-relationships/[id] error:", error);
    return NextResponse.json(
