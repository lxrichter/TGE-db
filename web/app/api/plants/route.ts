...
async function generateNextPlantId() {
  const db = await getDb();

  const row = await db.get<{ max_num: number | null }>(`
    SELECT MAX(CAST(SUBSTR(plant_id, 5) AS INTEGER)) AS max_num
    FROM plants
    WHERE plant_id LIKE 'PLT-%'
  `);

  const nextNum = (row?.max_num ?? 0) + 1;
  return `PLT-${String(nextNum).padStart(6, "0")}`;
}
  ...
  try {
    ...
    const plants = await db.all(`
      SELECT
        ...
        p.capacity_running_mw,
        p.project_phase,
        p.plant_technology,
        p.research_status,
        p.review_status,
        p.promoted_from_project_id,
        p.promoted_at,
        p.location_x,
        p.location_y,
...
export async function POST(req: Request) {
  try {
    ...
    const plant_id = await generateNextPlantId();
