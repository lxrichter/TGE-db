...
import {
  updateArticleFactCandidates,
  type ArticleFactCandidateAction,
...
function parseAction(value: unknown): ArticleFactCandidateAction | null {
...
export async function PATCH(req: Request) {
  try {
    ...
    const result = await updateArticleFactCandidates({
      candidateIds,
      action,
      actorUserId: user.id,
    });
    ...
    console.error("PostgreSQL article fact candidate update error:", error);
    return NextResponse.json(
      {
        success: false,
        error: "Failed to update PostgreSQL article fact candidates.",
      },
      { status: 500 }
    );
  }
